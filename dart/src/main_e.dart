...
void placeDistrict(
  ...
  currentCell.row = initialCell.row;
  currentCell.column = initialCell.column;

  if (projects[19].canPlace(city, currentCell)) {
    projects[19].place(city, currentCell);
  }
  ...
  for (int row = 0; row < 25; row++) {
    for (int column = 0; column < 27; column++) {
      ...
      if (projects[1].canPlace(city, currentCell)) {
        projects[1].place(city, currentCell);
      }
    }
  }
}
...
void placeDistricts(
  ...
  for (int row = 0; row < city.rows; row += 25) {
    for (int column = 0; column < city.columns; column += 27) {
      placeDistrict(
        city,
        new Cell(row, column),
        projects,
